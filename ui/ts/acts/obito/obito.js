...
function openPrintPreview() {
    ...
    const cidadeCartorio = (cidadeMatch ? cidadeMatch[1] : '') ||
    ...
    if (useInline) {
        ...
        if (body) {
            ...
            while (inline.firstChild) {
                ...
                const hasDebug = node.querySelector &&
                    (node.querySelector('#debug-invalid') ||
                        node.querySelector('#debug-alerts') ||
                        node.querySelector('#debug-matricula-base'));
                if (hasJson && paneJson)
                    paneJson.appendChild(node);
                else if (hasXml && paneXml)
                    paneXml.appendChild(node);
                else if (hasPrint && paneXml)
                    paneXml.appendChild(node);
                else if (hasDebug && paneDebug)
                    paneDebug.appendChild(node);
                else if (paneConfig)
                    paneConfig.appendChild(node);
                else
...
function updateOutputs() {
    ...
    if (xmlEl)
        xmlEl.value = toXml(data, 'certidao_obito', 0);
...
function applyState(input, field, state) {
    ...
    if (input)
        input.classList.toggle('invalid', state === 'empty' || state === 'invalid');
}

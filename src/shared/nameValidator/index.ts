...
async function loadCsvData() {
	...
	for (const p of nomesPaths) {
		try {
			...
			break;
		} catch (e) { tried.push(p); console.log('loadCsvData: failed to load nomes from', p); }
	}
	for (const p of gruposPaths) {
		try {
			...
			break;
		} catch (e) { tried.push(p); console.debug('loadCsvData: failed to load grupos from', p); }
	}
	if (!nomesText && !gruposText) console.log('loadCsvData: no remote data found, will use defaults', tried);
...
export class NameDictionaryRepository {
	...
	addException(value) {
		...
		this.save(data);
		try { console.log('NameDictionaryRepository.addException -> added', normalized); } catch (e) { /* ignore */ }
	}
...
export function createNameValidator(opts: CreateNameValidatorOptions = {}) {
	...
	const readyPromise = loadCsvData()
		.then((data) => {
			...
			ready = true; try { console.log('createNameValidator -> ready, base length', base.length); } catch (e) { /* ignore */ }
		})
		.catch(() => {
			ready = false;
		});

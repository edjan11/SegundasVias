...
export function getFieldLabel(input: HTMLElement): string {
	...
	if (!field) return (input as any)?.getAttribute?.('data-bind') || (input as any)?.name || (input as any)?.id || '';
	const label = field.querySelector('label');
	const text = label?.textContent?.trim();
	return text || (input as any)?.getAttribute?.('data-bind') || (input as any)?.name || (input as any)?.id || '';
}

...
function formatMatricula(matricula) {
  ...
  if (digits.length < 32) return fallback(matricula, 'NÃO CONSTA');
...
function formatBens(value) {
  ...
  if (!v || v === 'IGNORADA') return 'NÃO CONSTA';
  if (v === 'NAO') return 'NAO';
  return v;
}

import { escapeHtml } from '../../prints/shared/print-utils.js';
...
<center>
	...
	<span style="font-size: 10pt; font-weight: bold">${escapeHtml(
    fallback(normalizeUpper(registro.nome_completo)),
  )}</span><br>
	<div style="width: 80%">
		<table class="tabela">
			<tr>
		...
		<table class="tabela">
            ...
            fallback(normalizeTime(registro.hora_falecimento), 'N/C'),
          )} hora(s)</div>
					</fieldset>
				</td>
			</tr>
		...
		<table class="tabela">
			<tr>
				...
				<fieldset>
					...
					<div class="novomodelo">${escapeHtml(
            fallback(normalizeUpper(registro.nome_ultimo_conjuge_convivente)),
          )}</div>
					</fieldset>
				</td>
			</tr>
		...
		<table class="tabela">
			<tr>
				<td>
					...
					<div class="novomodelo">${escapeHtml(
            fallback(normalizeUpper(registro.local_sepultamento_cremacao)),
          )}</div>
				</fieldset>
				</td>
				<td>
				<fieldset>
					...
					<div class="novomodelo">${escapeHtml(
            fallback(normalizeUpper(registro.municipio_sepultamento_cremacao)),
          )}</div>
				</fieldset>
				</td>
				<td>
		...
		<table class="tabela">
			<tr>
				<td>
					...
					<div class="novomodelo" style="font-size: 95% !important">${escapeHtml(
            formatAnotacoes(registro.anotacoes_cadastro),
          )}</div>
					</fieldset>
				</td>
			</tr>
			...
			<tr>
				<td class="rodape-esquerdo">
					<div>
						...
						<div>${escapeHtml(cidadeCartorio)}${
    escapeHtml(ufCartorio) && cidadeCartorio ? '-' : ''
  }${escapeHtml(ufCartorio)}</div>
						<br>

// mapperHtmlToJson.ts: Migração estrutural do bundle para TypeScript
// Mapper HTML -> JSON extraído do bundle. Usa helpers internos para evitar acoplamento.
import { normalizeDate } from '../shared/validators/date';
import { normalizeCpf, isValidCpf, formatCpf } from '../shared/validators/cpf';

function getInputValue(name: string, root: Document | HTMLElement = document): string {
  const el = (root as any).querySelector(`[name="${name}"]`);
  if (!el) return '';
  if (el.tagName === 'TEXTAREA') return (el as any).value || '';
  return (el as any).value || '';
}
function getSelectValue(name: string, root: Document | HTMLElement = document): string {
  const el = (root as any).querySelector(`select[name="${name}"]`);
  if (!el) return '';
  return (el as any).value || '';
}
function getSelectText(name: string, root: Document | HTMLElement = document): string {
  const el = (root as any).querySelector(`select[name="${name}"]`);
  if (!el) return '';
  const opt = el.selectedOptions && el.selectedOptions[0];
  return opt ? opt.textContent.trim() : '';
}
function getCheckboxValue(name: string, root: Document | HTMLElement = document): boolean {
  const el = (root as any).querySelector(`[name="${name}"]`);
  if (!el) return false;
...
function mapRegimeBens(value: string): string {
...
function extractSelo(text: string): { selo: string; cod_selo: string } {
...
function buildRgItem(numero: string, data: string, orgao: string) {
...
export function mapperHtmlToJson(root: Document | HTMLElement = document) {
  const cnsInput = (root as any).querySelector('input[data-bind="certidao.cartorio_cns"]');
  if (cnsInput) {
    (cnsInput as any).value = '163659';
...
try {
  ...
  (win as any).App.mapperHtmlToJson = (win as any).App.mapperHtmlToJson || mapperHtmlToJson;
} catch (e) {
  // noop
}

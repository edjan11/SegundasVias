...
// src/acts/obito/mapperHtmlToJson.ts
...
// src/shared/nameValidator/index.ts
...
// src/acts/obito/printTemplate.ts
var MONTHS = [
  "JANEIRO",
  "FEVEREIRO",
  "MARCO",
...
function setupValidation() {
    ...
    document.querySelectorAll("input[data-cpf]").forEach((input) => {
      ...
      input.addEventListener("blur", () => {
        ...
        if (input.value && (!digits || !isValidCpf(digits)))
          setFieldHint(field, "CPF inv\xE1lido");
  ...
  document.querySelectorAll("select[data-required], textarea[data-required], input[data-required]").forEach((input) => {
    if (input.hasAttribute("data-date") || input.hasAttribute("data-time") || input.hasAttribute("data-cpf") || input.hasAttribute("data-name-validate"))
      return;
    const field = resolveField(input);
    const required = input.hasAttribute("data-required");
    const handler = () => {

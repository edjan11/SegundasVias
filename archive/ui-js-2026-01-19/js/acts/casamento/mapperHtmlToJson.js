...
function normalizeCpfFields(raw) {
    ...
    if (!digits)
        return { cpf: '', cpf_sem_inscricao: true };
...
export function mapperHtmlToJson(root = document) {
    ...
    const noivoRg = buildRgItem(getInputValue('numeroRGNoivo', root), getInputValue('dataExpedicaoRGNoivo', root), getSelectText('idOrgaoExpedidorRGNoivo', root));
    const noivaRg = buildRgItem(getInputValue('numeroRGNoiva', root), getInputValue('dataExpedicaoRGNoiva', root), getSelectText('idOrgaoExpedidorRGNoiva', root));
    const conjuges = [
        {
            ...
            genitores: buildGenitores(getInputValue('nomePaiNoivo', root), getInputValue('nomeMaeNoivo', root)),
        },
        {
            ...
            genitores: buildGenitores(getInputValue('nomePaiNoiva', root), getInputValue('nomeMaeNoiva', root)),
        },
    ];
    const anotacoes = {
        primeiro_conjuge: [],
    ...
    if (noivoRg)
        anotacoes.primeiro_conjuge.push(noivoRg);
    if (noivaRg)
        anotacoes.segundo_conjuge.push(noivaRg);
    return {
        certidao: {
            plataformaId,
